...
sed "s/{{.Values.app.label}}/app=\s${JOB_NAME}/g" ${JENKINS_HOME}/helm_template/cicd_sample/templates/service.yaml | sed "s/{{.Values.service.name}}/${JOB_NAME}/g" >> ${JENKINS_HOME}/${JOB_NAME}_helm_charts/cicd_sample/templates/${JOB_NAME}-service.yaml
